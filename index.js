...
function validateLesson(lesson) {
    const professorConflict = schedule.find((l) => l.professorId === lesson.professorId && l.dayOfWeek === lesson.dayOfWeek && l.timeSlot === lesson.timeSlot && l.id != lesson.id);
    if (professorConflict) {
        return { type: "ProfessorConflict", lessonDetails: professorConflict };
    }
    const classroomConflict = schedule.find((l) => l.classroomNumber === lesson.classroomNumber && l.dayOfWeek === lesson.dayOfWeek && l.timeSlot === lesson.timeSlot && l.id != lesson.id);
...
function showProfessorSchedule() {
    ...
    if (professorLessons.length > 0) {
        let scheduleHTML = '<table><thead><tr><th>Course</th><th>Day</th><th>Time</th><th>Classroom</th></tr></thead><tbody>';
...
//Додані аудиторії
classrooms.push({ number: "101", capacity: 50, hasProjector: true }, { number: "104", capacity: 30, hasProjector: false }, { number: "202", capacity: 35, hasProjector: false }, { number: "215", capacity: 40, hasProjector: true }, { number: "326", capacity: 45, hasProjector: true }, { number: "335", capacity: 40, hasProjector: false });
//Додані предмети
courses.push({ id: 1, name: "Algorithms", type: "Lab" }, { id: 2, name: "Discrete Math", type: "Practice" }, { id: 3, name: "Data Structures", type: "Lab" });
//Додані викладачі
...
addLesson({ id: 3, courseId: 2, professorId: 4, classroomNumber: "326", dayOfWeek: "Wednesday", timeSlot: "10:15-11:45" });
addLesson({ id: 4, courseId: 3, professorId: 4, classroomNumber: "215", dayOfWeek: "Friday", timeSlot: "15:45-17:15" });
addLesson({ id: 5, courseId: 1, professorId: 4, classroomNumber: "335", dayOfWeek: "Thursday", timeSlot: "12:15-13:45" });
populateScheduleTable();
